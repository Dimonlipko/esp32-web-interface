...
=======
#define INVERTER_PORT Serial
#define INVERTER_RX 4
#define INVERTER_TX 5
#define UART_TIMEOUT (100 / portTICK_PERIOD_MS)
#define UART_MESSBUF_SIZE 100
#ifndef LED_BUILTIN 
#define LED_BUILTIN 8 
...
*/
  // SPIFFS initialization
    DBG_OUTPUT_PORT.println("Initializing SPIFFS...");
  if (!SPIFFS.begin(true)) {
    DBG_OUTPUT_PORT.println("Failed to mount SPIFFS");
  ...
  WiFi.begin();
  DBG_OUTPUT_PORT.println("WiFi.status() ");
  DBG_OUTPUT_PORT.println(WiFi.status());
  //#endif
  //WiFi.mode(WIFI_STA); 
  //const char* ssid = "Keenetic-8390";
  //const char* password = "VsWwY6pQ";
  //WiFi.begin(ssid, password);

  // Перевірка підключення
  if (WiFi.status() != WL_CONNECTED) {
    DBG_OUTPUT_PORT.println("Не вдалося підключитися до роутера.");
    DBG_OUTPUT_PORT.println("Створення точки доступу...");

    // Створення точки доступу
    //WiFi.mode(WIFI_AP); // або WIFI_AP_STA
    //WiFi.softAP("MyESP32AP", "mypassword"); 

    DBG_OUTPUT_PORT.print("Точка доступу створена. SSID: ");
    DBG_OUTPUT_PORT.println(WiFi.softAPSSID());
    DBG_OUTPUT_PORT.print("IP адреса: ");
    DBG_OUTPUT_PORT.println(WiFi.softAPIP());
  } else {
    DBG_OUTPUT_PORT.println("Підключено до роутера.");
    DBG_OUTPUT_PORT.println("IP адреса: ");
...
void loop(void){
  ...
  if(millis() - timer_loop >= 1000)
  {
    timer_loop = millis();
    DBG_OUTPUT_PORT.println("hello loop");
  }
